...
void ProcRsp::setImage(const QImage & img)
{
   delete matrix_;
   matrix_ = 0;
...
QPair<QRect, QRect> ProcRsp::getHoleCards(bool * ok)
{
...
int ProcRsp::countCheckLetters(const QImage & imgCheck)
{
   //очерняем
...
bool ProcRsp::checkHueButton(const QImage & imgBtn, int minHue, int maxHue, int rectMin)
{
...
bool ProcRsp::checkHueButton(const QImage & imgBtn, int minHue, int maxHue, int rectMin, QRect & rect)
{
   const int btnWidth = imgBtn.width();
   const int btnHeight = imgBtn.height();
   bool hasBtn = false;
