...
void Executor::timerEvent(QTimerEvent *)
{
   ...
   if (BtnWnd)
   {
      ...
      hasCheck = ProcRsp::checkHueButton(imgBtn, minCheckHue, maxCheckHue, 
         rectMin, checkRect);
      if (!hasCheck)
      {
         //ищем кнопку Fold и Call
         hasFold = ProcRsp::checkHueButton(imgBtn, minFoldHue, maxFoldHue, 
            rectMin, foldRect);
         hasCall = ProcRsp::checkHueButton(imgBtn, minCallHue, maxCallHue, rectMin);
