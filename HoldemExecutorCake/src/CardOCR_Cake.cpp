...
QString CardOCR::nominal(const QImage * img)
{
   ...
   QList<int> areas = closedAreas(imgMatrix);
   ...
   if (isLeftQueen && isRightSix && cnt >= 1)
      return "Q";
   if ((isLeftQueen || isRightSix) && cnt == 2)
      return "Q";

   bool isLeftSix   = isSixProfile(leftProfile);
   //bool isRightSix  = isSixProfile(rightProfile);

   if (isLeftSix && isRightSix && cnt == 1)
      return "T";
...
//true 0.214286 
   if (profile.count() == 3)
   {
      if ((profile.at(0).item == true) &&
          ...
          (profile.at(1).value > profile.at(0).value) &&
          (profile.at(1).value > profile.at(2).value))
      return true;
   }
   ...
   if (profile.count() >= 3)
   {
      for (int i = 0; i < profile.count() - 2; i++)
      {
         if ((profile.at(i).item == false)    &&
             (profile.at(i + 1).item == true) &&
             (profile.at(i + 2).item == false) &&
             ((profile.at(i + 1).value + 0.01) >= profile.at(i).value) &&
             ((profile.at(i + 1).value + 0.01) >= profile.at(i + 2).value))
         {
            idx = i;
            break;
         }
...
bool CardOCR::isRightQueenProfile(const QVector<ProfileItem> & profile)
{
   ...
   if (profile.count() == 2)
   {
      if ((profile.at(1).value >  profile.at(0).value * 5.) &&
          (profile.at(0).item == true) && (profile.at(1).item == false))
          isQueen = true;
      if ((profile.at(0).value >  profile.at(1).value * 4.) &&
          (profile.at(0).item == true) && (profile.at(1).item == false))
          isQueen = true;
...
QList<int> CardOCR::closedAreas(const BoolMatrix & bmatr_in)
{
...
void CardOCR::fillMatrix(BoolMatrix & bm, int x, int y)
{
   if (x < 0 || y < 0) 
      return;
