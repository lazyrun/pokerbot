...
void Executor::timerEvent(QTimerEvent *)
{
   ...
   if (BtnWnd)
   {
      ...
      hasCheck = ProcParty::checkHueButton(imgBtn, minCheckHue, maxCheckHue, 
         rectMin, checkRect);
      if (!hasCheck)
      {
         //ищем кнопку Fold и Call
         hasFold = ProcParty::checkHueButton(imgBtn, minFoldHue, maxFoldHue, 
            rectMin, foldRect);
         hasCall = ProcParty::checkHueButton(imgBtn, minCallHue, maxCallHue, rectMin);
