...
void Executor::timerEvent(QTimerEvent *)
{
   ...
   if (BtnWnd)
   {
      ...
      hasCheck = ProcTitan::checkHueButton(imgBtn, minCheckHue, maxCheckHue, 
         rectMin, checkRect);
      if (!hasCheck)
      {
         //ищем кнопку Fold и Call
         hasFold = ProcTitan::checkHueButton(imgBtn, minFoldHue, maxFoldHue, 
            rectMin, foldRect);
         hasCall = ProcTitan::checkHueButton(imgBtn, minCallHue, maxCallHue, rectMin);
