...
void Proc770::setImage(const QImage & img)
{
   delete matrix_;
   matrix_ = 0;
...
QPair<QRect, QRect> Proc770::getHoleCards(bool * ok)
{
...
int Proc770::countCheckLetters(const QImage & imgCheck)
{
   //очерняем
...
bool Proc770::checkHueButton(const QImage & imgBtn, int minHue, int maxHue, int rectMin)
{
...
bool Proc770::checkHueButton(const QImage & imgBtn, int minHue, int maxHue, int rectMin, QRect & rect)
{
   const int btnWidth = imgBtn.width();
   const int btnHeight = imgBtn.height();
   bool hasBtn = false;
